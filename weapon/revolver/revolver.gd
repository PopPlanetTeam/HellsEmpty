...
var _projectile_scene: PackedScene = preload("res://weapon/revolver/projectile/revolver_projectile.tscn")
...
func _process(_delta) -> void:
	...
	if Input.is_action_pressed("shoot"):
		...
		var new_shot: RevolverProjectile = shot.duplicate()
		...
		new_shot.scale = self.scale * (GlobalData.player.scale if GlobalData.player else 1.0)
